    ...
    <>
      <tr className="border-b border-border hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => setExpanded((v) => !v)}>
        <td className="px-4 py-3 text-sm font-medium">
          <div className="flex items-center gap-2"><User className="h-3.5 w-3.5 text-muted-foreground shrink-0" />{lead.firstName} {lead.lastName}</div>
        </td>
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
            {(lead.status === "error" || lead.status === "quoted") && (
              <button onClick={async (e) => { e.stopPropagation(); setRerunning(true); await onRerun(lead._id); setRerunning(false); }} disabled={rerunning} title="Reset and re-queue" className="text-muted-foreground hover:text-blue-500 transition-colors disabled:opacity-40">
                <RotateCcw className={`h-3.5 w-3.5 ${rerunning ? "animate-spin" : ""}`} />
              </button>
            )}
            <button onClick={(e) => { e.stopPropagation(); onEdit(lead); }} title="Edit lead" className="text-muted-foreground hover:text-foreground transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
            <button onClick={(e) => { e.stopPropagation(); if (confirm(`Delete lead for ${lead.firstName} ${lead.lastName}?`)) onDelete(lead._id); }} title="Delete lead" className="text-muted-foreground hover:text-destructive transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
      ...
      {expanded && (
        <tr className="bg-muted/10 border-b border-border">
          <td colSpan={8} className="px-6 py-3">
            ...
            {quote.quoteId && <p className="text-sm text-muted-foreground">Quote #: {quote.quoteId}</p>}
            {quote.coverageDetails && <pre className="text-xs text-muted-foreground mt-1 overflow-x-auto">{JSON.stringify(quote.coverageDetails, null, 2)}</pre>}
  ...
  return (
    <PageContainer variant="scroll">
      ...
      {!hasNatGenCreds && (
        <div className="flex items-start gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3">
          ...
          <div className="flex-1 text-sm">
            ...
            <p className="text-muted-foreground mt-0.5">The quoting agent needs your NatGen agent username and password to log in and run quotes.</p>
          </div>
          <Link href="/settings">
      ...
      {stats && (
        <div className="stats-grid">
          ...
          <div className="rounded-xl border bg-card p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-1"><TrendingUp className="h-4 w-4" /><span className="section-heading">By Type</span></div>
            <div className="flex gap-2 mt-1">
              ...
              {Object.keys(stats.byType).length === 0 && <p className="text-sm text-muted-foreground">No data yet</p>}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-1 border-b border-border">
        <button onClick={() => setActiveTab("leads")} className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px ${activeTab === "leads" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
          Leads{leads && leads.length > 0 && <span className="ml-2 text-xs bg-muted rounded-full px-1.5 py-0.5">{leads.length}</span>}
        </button>
        <button onClick={() => setActiveTab("quotes")} className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px ${activeTab === "quotes" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
          Quote Results{quotes && quotes.length > 0 && <span className="ml-2 text-xs bg-muted rounded-full px-1.5 py-0.5">{quotes.length}</span>}
        </button>
      </div>
        ...
        <div className="rounded-xl border bg-card overflow-hidden">
          {leads === undefined ? (
            ...
            <div className="p-8 text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">No leads yet. Click "Add Lead" to add your first insurance lead.</p>
          ...
          <div className="flex items-center gap-3">
            <div className="flex gap-1 rounded-lg border p-1 bg-muted/30">
              {(["all", "auto", "home"] as const).map((t) => (
                <button key={t} onClick={() => setTypeFilter(t)} className={`px-3 py-1 text-xs rounded-md transition-colors capitalize ${typeFilter === t ? "bg-primary text-primary-foreground font-medium" : "text-muted-foreground hover:text-foreground"}`}>
                  {t === "all" ? "All Types" : t}
                </button>
              ))}
            </div>
            <div className="flex gap-1 rounded-lg border p-1 bg-muted/30">
              {(["all", "success", "error"] as const).map((s) => (
                <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1 text-xs rounded-md transition-colors capitalize ${statusFilter === s ? "bg-primary text-primary-foreground font-medium" : "text-muted-foreground hover:text-foreground"}`}>
                  {s === "all" ? "All Status" : s}
                </button>
              ))}
            </div>
            <span className="text-xs text-muted-foreground ml-auto">{filteredQuotes.length} record{filteredQuotes.length !== 1 ? "s" : ""}</span>
          </div>
          <div className="rounded-xl border bg-card overflow-hidden">
            {quotes === undefined ? (
              ...
              <div className="p-8 text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">{quotes.length === 0 ? "No quote results yet. Add leads and run the agent to get started." : "No quotes match the current filters."}</p>
              </div>
            ) : (
              <table className="w-full">
                <thead>
