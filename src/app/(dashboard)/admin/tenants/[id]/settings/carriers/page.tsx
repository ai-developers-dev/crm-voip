...
import { Loader2, AlertCircle, CheckCircle, Info, ArrowLeft, Eye, Plus, Trash2, ChevronRight, ToggleLeft, ToggleRight, Download, Chrome } from "lucide-react";
import { useParams, useRouter } from "next/navigation";
import Link from "next/link";

export default function AdminTenantCarriersPage() {
  ...
  useEffect(() => {
    ...
    if (tenant.agencyTypeId) {
      setSelectedAgencyTypeId(tenant.agencyTypeId);
    }

    if (tenant.agencyTypeId && existingCarriers === undefined) return;
    if (tenant.agencyTypeId && existingProducts === undefined) return;
    if (tenant.agencyTypeId && existingCommissions === undefined) return;
    ...
    setHydrated(true);
  }, [hydrated, tenant, existingCarriers, existingProducts, existingCommissions]);
  ...
  const handleSubmit = async () => {
    if (!tenant?.clerkOrgId || !selectedAgencyTypeId) return;
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-4 flex-1">
        ...
        <div>
          ...
          <p className="text-on-surface-variant">
            Select the carriers and products for {tenant.name}
          </p>
        </div>
                    ...
                    {activeCarriers.map((carrier) => {
                      ...
                      return (
                        <div key={carrier._id} className="break-inside-avoid">
                          <div className="flex items-center gap-2 py-1.5 px-2 rounded-xl hover:bg-surface-container-high/50">
                            <button
                              type="button"
                            ...
                            <div className="pl-10 space-y-0.5 pb-1">
                              {carrierProducts.length === 0 ? (
                                ...
                                carrierProducts.map((product) => (
                                  <label
                                    key={product._id}
                                    className="flex items-center gap-2 py-1 px-1.5 rounded-xl hover:bg-surface-container-high/30 cursor-pointer"
                                  >
                                    <Checkbox
            ...
            <CardContent className="px-4 pb-4 pt-0 space-y-3">
              {saleTypes && saleTypes.length > 0 && (
                <div className="space-y-0.5">
                  {saleTypes.map((st) => (
                    <div key={st._id} className="flex items-center justify-between py-1.5 px-2 rounded-xl hover:bg-surface-container-high/50 group -mx-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && newSaleTypeName.trim() && tenant?._id) {
                      e.preventDefault();
                      createSaleType({ organizationId: tenant._id, name: newSaleTypeName.trim() });
                      setNewSaleTypeName("");
                    }
                  }}
                />
                <Button
                  ...
                  disabled={!newSaleTypeName.trim() || !tenant?._id}
                  onClick={() => {
                    if (tenant?._id && newSaleTypeName.trim()) {
                      createSaleType({ organizationId: tenant._id, name: newSaleTypeName.trim() });
                      setNewSaleTypeName("");
                    }
                  }}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-2xl border bg-surface-container/30 p-4 space-y-3">
            ...
            <div className="rounded-2xl border bg-surface-container/30 p-4 space-y-2">
