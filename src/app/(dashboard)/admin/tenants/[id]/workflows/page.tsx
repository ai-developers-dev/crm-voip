...
import { Loader2, Plus, Workflow, Users, Activity, Trash2, Phone, MessageSquare, Calendar, BarChart3, Bot, Settings, Columns3, ClipboardCheck } from "lucide-react";
      ...
      <div className="shrink-0 border-b bg-background px-6 py-3">
        <div className="flex items-center justify-between">
          <nav className="flex items-center gap-1">
            ...
            <Link href={`/admin/tenants/${tenant._id}/workflows`}><Button variant="secondary" size="sm" className="gap-2"><Workflow className="h-4 w-4" />Workflows</Button></Link>
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        {workflows && workflows.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4 max-w-sm">Create your first workflow to automate SMS, tasks, tags, and more.</p>
            <Link href={`/admin/tenants/${tenant._id}/workflows/new`}>
          ...
          <div className="space-y-2">
            {(workflows ?? []).map((wf) => {
              ...
              return (
                <Link key={wf._id} href={`/admin/tenants/${tenant._id}/workflows/${wf._id}`} className="flex items-center gap-3 group rounded-lg border bg-card p-4 hover:shadow-sm transition-shadow">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      <span className="text-xs text-muted-foreground">{wf.steps.length} step{wf.steps.length !== 1 ? "s" : ""}</span>
                      <span className="flex items-center gap-1 text-xs text-muted-foreground"><Users className="h-3.5 w-3.5" />{wfStats?.total ?? 0} contacts</span>
                  ...
                  <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (confirm("Delete this workflow?")) removeWorkflow({ id: wf._id }); }} className="p-1.5 rounded text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100"><Trash2 className="h-4 w-4" /></button>
                </Link>
              );
            })}
          </div>
