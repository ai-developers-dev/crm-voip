...
export default function TenantEditWorkflowPage() {
  const params = useParams();
  const tenantId = params.id as string;
  const wfId = params.wfId as string;
  ...
  const workflow = useQuery(
    ...
    wfId ? { id: wfId as Id<"workflows"> } : "skip"
  );

  if (tenant === undefined || workflow === undefined) {
    return (
  ...
  if (!tenant || !workflow) return null;
